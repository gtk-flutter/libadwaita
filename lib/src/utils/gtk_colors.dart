...
class GnomeTheme {
  ...
  ThemeData data(context) {
    ThemeData themedata = ThemeData(
        ...
        elevatedButtonTheme:
            ElevatedButtonThemeData(style: ElevatedButton.styleFrom(primary: buttonFg, onPrimary: textColor)),
        textTheme: TextTheme(bodyText2: TextStyle(color: textColor), bodyText1: TextStyle(color: textColor)),
        primaryColor: themeBaseColor);
    return themedata.copyWith(colorScheme: themedata.colorScheme.copyWith(secondary: textColor));
  }
  ...
  void parse() {
    ...
    textColor = getBaseColor("theme_text_color") ??
        (theme.contains('dark') || (themeBgColor != null && themeBgColor!.computeLuminance() <= 0.5)
            ? Colors.white
            : Colors.black);
...
Color getGtkColor({
  ...
  switch (colorTheme) {
    ...
    default:
      ...
      theme.loadFromFile();
      theme.parse();
      return _system(theme)[describeEnum(colorType)] ?? _adwaita(isDark)[describeEnum(colorType)]!;
  }
}
...
Map<String, Color?> _system(GnomeTheme theme) {
  return {
    ...
    'headerButtonBackgroundBottomHover': theme.themeSelectedBG?.withOpacity(0.2),
...
Map<String, Color> _adwaita(bool isDark) {
  return {
    'windowControlIconColor': isDark ? const Color(0xFFEEEEEC) : const Color(0xFF2E3436),
    'canvas': isDark ? const Color(0xFF353535) : const Color(0xFFF6F5F4),
    'headerBarBackgroundTop': isDark ? const Color(0xFF2B2B2B) : const Color(0xFFE1DEDB),
    'headerBarBackgroundBottom': isDark ? const Color(0xFF262626) : const Color(0xFFDAD6D2),
    'headerBarTopBorder': isDark ? const Color(0xFF383838) : const Color(0xFFF9F8F8),
    'headerBarBottomBorder': isDark ? const Color(0xFF070707) : const Color(0xFFBFB8B1),
    'headerButtonBackgroundTop': isDark ? const Color(0xFF383838) : const Color(0xFFF6F5F3),
    'headerButtonBackgroundBottom': isDark ? const Color(0xFF333333) : const Color(0xFFEEECEA),
    'headerButtonBackgroundTopHover': isDark ? const Color(0xFF383838) : const Color(0xFFF6F5F3),
    'headerButtonBackgroundBottomHover': isDark ? const Color(0xFF333333) : const Color(0xFFEEECEA),
    'headerButtonBorder': isDark ? const Color(0xFF1B1B1B) : const Color(0xFFCEC8C3),
    'headerButtonPrimary': isDark ? const Color(0xFFEEEEEC) : const Color(0xFF393E40),
    'headerSwitcherTabPrimary': isDark ? const Color(0xFFE6E6E4) : const Color(0xFF2E3436),
    'headerSwitcherTabBackground': isDark ? const Color(0xFF1E1E1E) : const Color(0xFFD5D1CD),
    'headerSwitcherTabBorder': isDark ? const Color(0xFF1B1B1B) : const Color(0xFFCDC7C2),
  };
}
